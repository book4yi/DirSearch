...
async def scan(url, sem):
    async with sem:
                    ...
                    if status in [200]:
                        ...
                        async with session.get(url=url2, allow_redirects=False, headers=headers, verify_ssl=False,
                            ...
                            if resp2.status == [200, 404]:
                                ...
                                alive_40X_30X_list.add(url + "\t%d\t%s\n" % (status, title))
                                if r_scan:
        ...
        if len(status_200_list):
            ...
            f1.close()
        if len(alive_40X_30X_list):
            f2 = open(path2, 'w+', encoding='utf-8')
            f2.writelines(alive_40X_30X_list)
            f2.close()
...
if __name__ == "__main__":
    ...
    print("403-url数量:", len(alive_40X_30X_list))
